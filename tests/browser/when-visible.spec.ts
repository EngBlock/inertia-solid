...
test('reloads selected props only when visible and only once by default', async ({ page }) => {
  ...
  const response = page.waitForResponse(
    (result) =>
      result.url().includes('/when-visible') && result.request().headers()['x-inertia-partial-data'] === 'foo',
  )
...
test('always reloads, exposes fetching, and reads current params lazily', async ({ page }) => {
  ...
  let response = page.waitForResponse(
    (result) => result.url().includes('count=0') && result.url().includes('param=initial'),
  )
  ...
  response = page.waitForResponse(
    (result) => result.url().includes('count=1') && result.url().includes('param=initial'),
  )
  await page.evaluate(() => window.scrollTo(0, 13000))
  ...
  response = page.waitForResponse(
    (result) => result.url().includes('count=2') && result.url().includes('param=updated'),
  )
  await page.evaluate(() => window.scrollTo(0, 13000))
  await response
...
test('cleans up and observes again after repeated mounting', async ({ page }) => {
  ...
  const response = page.waitForResponse(
    (result) =>
      result.url().includes('/when-visible') && result.request().headers()['x-inertia-partial-data'] === 'delayed',
  )
