...
4. Run all applicable shared specs in Chromium and WebKit; run `SSR=true` separately. Firefox remains outside the matrix while Playwright 1.58's bundled runner hangs before page creation.
...
11. **Quality gates:** package build/type generation and ES2022 check pass; all applicable shared Playwright specs pass in Chromium/WebKit and SSR mode; Solid-specific owner/diagnostic tests pass. Firefox is not claimed while its pinned Playwright runner cannot start reliably.
